...
DyadBoot is an R package that provides tools for analysing experimental dyad data at the individual level.  It randomly assigns the role of "focal" or "opposite" to each individual of each dyad and performs bootstrapping of chosen model (currently supports lm, glm, lmer, glmer, and glm.nb) - reassigning the "focal" and "opposite" roles at each iteration of the bootstrapping to mitigate sampling bias. Output contains summary tables and Anova tables ('car' package) of all bootstapping iterations. Includes other functions for summary statistics and plotting capabilites (read vignette). ChatGPT-4 was used to generalise original functions in this package. The initial function for 'randBoot' was based on a statistical analysis methodology provided by Jack G. Rayner (featured in Rayner, J.G. and Bailey, N.W., 2019. Testing the role of same-sex sexual behaviour in the evolution of alternative male reproductive phenotypes. Animal Behaviour, 157, pp.5-11).
...
strucutred so that each row pertains to an individual
...
'randOne', and 'randMult' restructure the dataframe by randomly assigning the roles of "focal" and "opposite" to each individual of each dyad so that there is one row per dyad, with separate columns for the variables associated with the "focal" and "opposite" individual of each dyad.
'randOne' performs this once (useful for preliminary tests such as testing model assumptions), while 'randBoot' and 'randMult' perform this the number of times specified by the user. randOne and randMult simply generate the new dataframe(s) while randBoot runs a specified model on each generated dataframe.
...
1.  ‘randBoot’ - Randomly assigns the role of “focal” and “opposite” to
    ...
    chosen model (package only supports lm, glm, lmer, glmer, and glm.nb so
...
<!-- -->
    ...
    results <- DyadBoot::randBoot(data = data,
                               dyad_id_col = "your_dyad_column_name",
    ...
    #"focal_cols" and "opposite_cols" both take the original name of the variable, without the attribution of "focal" or "opposite". Must be the same variable. 
...
2.  ‘randOne’ - To only perform the random assignment of roles once, without
...
3.  ‘randMult’ - To perform the random assignment of roles a specified
...
4.  ‘repDataSet’ - Takes ‘randMult’ output as input and chooses the most
...
5.  ‘averages’ - Takes the output of DyadBoot::randBoot as input and
    ...
    these averages. Handles output from summary() and car::Anova() functions.
...
6.  ‘countPvals’ - Takes the output of DyadBoot::randBoot as input and
    returns the number of times each tested factor is statistically significant
    (p-value < 0.05). Handles output from summary() and car::Anova() functions.
...
7.  ‘anovaPvals’ - Extracts all P-values from the Anova output generated
...
8.  ‘MeanOfMeans’ - Calculates the overall mean of all bootstrap
...
9.  ‘histMeans’ - Histogram of distribution of means for specified level
...
10.  ‘histPvals’ - Histogram of the distribution of all p-values for the
...
11.  -   ‘plotBoot’ - Two grid plot. Left grid -raincloud plot (with
...
Here we use an example ficticious data set on cricket behaviour. "Aggression_Duration" is the dependent variable, and "Genotype" and "Song_occurrence" are the independent variables.
    ...
    results <- DyadBoot::randBoot(data = mydata, 
                                  dyad_id_col = "Trial_ID", # the name of the dyad id column in this case
                                  model_formula = model_formula,
                                  model_type = "lm", # linear model
                                  n_bootstraps = 1000, # number of iterations
                                  focal_cols = c("Aggression_Duration"), 
                                  opposite_cols = c("Aggression_Duration"))

    #"focal_cols" and "opposite_cols" both take the original name of the variable, without the "focal_" or "opposite_" prefixes. These must be the same variable (the dependent variable of interest).
